...
export default function WhySection() {
  ...
  return (
    <section ref={sectionRef} className="bg-gradient-to-b from-white to-amber-50/40 py-24">
      <div className="max-w-6xl mx-auto px-8">
        ...
        >
          <h2 className="text-3xl font-bold text-stone-900 mb-4">
            Why{" "}
            <span className="text-amber-700">코지?</span>
          </h2>
          ...
          {features.map((feature, idx) => {
            ...
            return (
              <Fragment key={feature.title}>
                <div className="flex flex-col items-center text-center w-48 flex-none">
                  {/* Icon box */}
                  <div
                    className={`w-20 h-20 rounded-2xl flex items-center justify-center mb-5 transition-all duration-700 ${feature.iconBg} ${
                      visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-5"
                    }`}
                    style={{ transitionDelay: `${itemDelay}s` }}
                  >
                    <Icon size={34} className={feature.iconColor} />
                  </div>

                  {/* Title */}
                  <h3
                    className={`font-bold text-stone-900 mb-2 transition-all duration-700 ${
                      visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-5"
                    }`}
                  ...
                  {/* Description */}
                  <p
                    className={`text-stone-500 text-sm leading-relaxed whitespace-pre-line transition-all duration-700 ${
                      visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-5"
                    }`}
